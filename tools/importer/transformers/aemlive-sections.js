...
/**
 * Transformer: aem.live section breaks and Section Metadata.
 * Inserts <hr> boundaries between the 7 template sections and Section Metadata
 * blocks for the two styled sections (s1 hero, s7 closing-cta — both
 * "colorful-gradient").
 *
 * Selectors come directly from page-templates.json template "home" sections[]
 * (each section.selector is DOM-verified against migration-work/cleaned.html).
 *
 * Follows the reference implementation: breaks are inserted in beforeTransform
 * (while every section element still exists, before parsers replace them) using
 * a temporary marker attribute; Section Metadata is inserted in afterTransform,
 * anchored to the surviving marker <hr>. Sections are iterated in reverse so
 * inserts never disturb the position of not-yet-processed sections.
 */
